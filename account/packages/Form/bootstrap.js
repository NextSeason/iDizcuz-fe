J.Package( {
    ...
    bindEvent : function() {
        ...
        $( 'form.signup input.email' ).on( 'input propertychange', function( e ) {
            ...
            reg.test( email ) ? $( 'form.signup .send-btn' ).removeClass( 'disabled' ) : $( 'form.signup .send-btn' ).addClass( 'disabled' );
        ...
        $( 'form.sign' ).on( 'click', '.to-signin', function( e ) {
            e.preventDefault();
            me.switchTo( 'signin' );
        } );

        $( 'form.sign' ).on( 'click', '.to-signup', function( e ) {
            e.preventDefault();
            me.switchTo( 'signup' );
        } );

        $( 'form.sign' ).on( 'click', '.forget', function( e ) {
            e.preventDefault();
            me.switchTo( 'forget' );
        } );
    },
    ...
    signup : function() {
        ...
        if( !uname.length ) {
            me.setTip( '请输入用户名', 'warn' ); 
            return false;
        }

        if( J.byteLen( uname ) > 18 ) {
            me.setTip( '用户名长度为1~6个字(三个字母等于一个字)', 'warn' ); 
            return false;
        }
        ...
        if( !passwd.length ) {
            me.setTip( '请输入密码', 'warn' );
            return false;
        }

        if( passwd.length > 20 || passwd.length < 6 ) {
            me.setTip( '密码长度需要在6~20字符之间', 'warn' );
            return false;
        }
        ...
        if( !vcode.length ) {
            me.setTip( '请输入验证码，若未收到验证码，请点击重新发送', 'warn' );
            return false;
        }
        ...
        $.ajax( {
            ...
            switch( errno ) {
                case 4 :
                    ...
                    break;
                case 5 : 
                    me.setTip( '该邮箱已存在，您可以直接<a href="###" class="to-signin">登录</a>', 'warn' );
                    break;
                default :
